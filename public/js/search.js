...
function searchUpdateHandler2(){
    if (this.status != 200) window.location = '';
    let response = JSON.parse(this.responseText);
    console.log(response)
    window.scroll({top: 0, behavior: 'smooth'});

    let questionsProfileDiv = document.querySelector('#pagination-item-1');
    questionsProfileDiv.innerHTML = response.html;
    
    updatePagination2();
}

function sendAdvancedSearchRequest2(page = 1) {
    //let searchInput = searchBar.querySelector("input[type='search']").value;
    
    let data = {
        'page': page,
    };
    sendAjaxGetRequest('get', '/api/user', data, searchUpdateHandler2);
    
    let url = 'profile?' + encodeForAjax(data)
    window.history.pushState({}, '', url);
}

function searchPagination2(event) {
    event.preventDefault();
    page = this.href.split('page=')[1]
    sendAdvancedSearchRequest2(page);
}

function updatePagination2() {
    
    document.querySelectorAll('.pagination a').forEach(
        paginationLink => { paginationLink.addEventListener('click', searchPagination2);});
